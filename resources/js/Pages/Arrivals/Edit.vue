...
const props = defineProps({
    ...
    landingSites: { type: Array, required: true },
    fishSpecies: { type: Array, required: true }
})

const catches = ref([])

if (props.arrival.catches && props.arrival.catches.length > 0) {
    catches.value = props.arrival.catches.map(c => ({
...
const form = useForm({
    ...
    average_price: props.arrival.average_price || '',
    waste_volume: props.arrival.waste_volume || '',
    fish_temperature: props.arrival.fish_temperature || '',
    hold_temperature: props.arrival.hold_temperature || '',
    status: props.arrival.status,
    approval_status: props.arrival.approval_status,
    notes: props.arrival.notes || '',
    is_processed: props.arrival.is_processed || false,
    catches: [],
})
...
const submit = () => {
    ...
    form.put(`/arrivals/${props.arrival.id}`)
}

const cancel = () => (window.location.href = '/arrivals')
</script>

<template>
    <AppLayout>
        <Head title="Edit Kedatangan Kapal - SAMOSIR" />
        
        <div class="max-w-4xl mx-auto">
            <div class="mb-4">
                <h1 class="text-xl font-bold text-gray-900 dark:text-white">Edit Kedatangan Kapal</h1>
                <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">
                    Edit data kedatangan kapal yang sudah dicatat
                </p>
            </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input v-model="form.origin" type="text" placeholder="Contoh: Laut Jawa" :class="['w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500', form.errors.origin ? 'border-red-500' : 'border-gray-300 dark:border-gray-600', 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white']" />
                            </div>
                        </div>
                    </div>
