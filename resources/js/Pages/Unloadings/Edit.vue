...
const form = useForm({
    arrival_id: props.unloading.arrival_id || '',
    reference_number: props.unloading.reference_number || '',
    syahbandar_id: props.unloading.syahbandar_id || '',
    captain_name: props.unloading.captain_name || '',
    identification_mark: props.unloading.identification_mark || '',
    unloading_date: props.unloading.unloading_date || '',
    unloading_time: props.unloading.unloading_time || '',
    sequence_number: props.unloading.sequence_number || '',
    registration_date: props.unloading.registration_date || '',
    bl_code: props.unloading.bl_code || '',
    landing_site_id: props.unloading.landing_site_id || '',
    notes: props.unloading.notes || '',
})
...
// Initialize selected arrival on mount
onMounted(() => {
    const arrival = props.arrivals.find(a => a.id === props.unloading.arrival_id)
    if (arrival) {
...
const submit = () => {
    form.put(`/unloadings/${props.unloading.id}`, {
        onSuccess: () => {
            form.reset()
        }
    })
...
<template>
    <AppLayout>
        <Head title="Edit Penimbangan Ikan - SAMOSIR" />

        <div class="max-w-4xl mx-auto">
            <div class="mb-4">
                <h1 class="text-xl font-bold text-gray-900 dark:text-white">Edit Penimbangan Ikan</h1>
                <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">
                    Edit data penimbangan ikan di pelabuhan
                </p>
            </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="md:col-span-2 relative" ref="dropdownContainer">
                            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
                                ...
                                <p class="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5">Pilih kapal dari data kedatangan yang sudah selesai</p>
                            </label>
                            <div class="relative">
                                <input
