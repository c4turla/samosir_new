...
const submit = () => {
    ...
    // Create a copy of the form data with filtered items
    const data = {
        ...form.data(),
        items: filteredItems
    }
    ...
    form.transform((data) => ({
        ...data,
        items: filteredItems,
        // Match controller's expected field names
        field_officer: form.officer 
    })).post('/equipment-services', {
        onSuccess: () => {
            form.reset()
        }
    })
...
<style scoped>
/* Optional styling to match the image's clean look */
